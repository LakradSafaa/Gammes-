...
const referentielItems: NavItem[] = [
  { label: "Gestion des listes", path: "/referentiels", icon: <ListTree size={17} /> },
...
export default function Sidebar() {
  ...
  const libraryActive = useMemo(
    () =>
      ...
      referentielItems.some((item) => item.path !== "/referentiels" && location.pathname.startsWith(item.path)),
    [location.pathname],
  );
    ...
    <aside className="sidebar">
      ...
      <div className="sidebar-scroll">
        ...
        <section className="sidebar-section">
          ...
          {librariesOpen && (
            <div className="sidebar-submenu">
              {referentielItems.map((item) => <NavigationItem key={item.path} item={item} />)}
            </div>
          )}
        </section>
        ...
        <section className="sidebar-section">
          ...
          {systemOpen && (
            <div className="sidebar-submenu">
              {systemItems.map((item) => <NavigationItem key={item.path} item={item} />)}
            </div>
          )}
        </section>
      </div>
