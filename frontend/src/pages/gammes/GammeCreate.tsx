...
type EtapeImageDraft = { localId: string; image_url: string; description: string };
type EtapeDraft = {
  localId: string; numero: number; titre: string; description: string;
...
export default function GammeCreate() {
  ...
  const addEtape = () => {
    setEtapes((current) => [...current, {
      localId: createLocalId(), numero: current.length + 1, titre: "", description: "",
      duree_minutes: 0, actions: [{ localId: createLocalId(), contenu: "" }], images: [],
    }]);
    setError("");
  };

  /** Modifie le titre, la description ou la durée d'une étape locale. */
  const updateEtape = (id: string, field: "titre" | "description" | "duree_minutes", value: string) => {
    setEtapes((current) => current.map((e) => e.localId === id ? {
      ...e, [field]: field === "duree_minutes" ? Math.max(0, Number(value) || 0) : value,
    } : e));
  };
  ...
  /** Lit une image locale (max. 3 Mo) et la conserve en Data URL avant sauvegarde. */
  const addEtapeImage = (etapeId: string, file?: File) => {
    if (!file) return;
    if (!file.type.startsWith("image/")) { setError("Le fichier sélectionné n'est pas une image."); return; }
    if (file.size > 3 * 1024 * 1024) { setError("Chaque image doit faire au maximum 3 Mo."); return; }
    const reader = new FileReader();
    reader.onload = () => {
      if (typeof reader.result !== "string") { setError("Impossible de lire l'image."); return; }
      setEtapes((current) => current.map((e) => e.localId === etapeId ? {
        ...e, images: [...e.images, { localId: createLocalId(), image_url: reader.result as string, description: "" }],
      } : e));
      setError("");
    };
  ...
  /** Modifie la description d'une image d'étape. */
  const updateEtapeImageDescription = (etapeId: string, imageId: string, description: string) => {
    setEtapes((current) => current.map((e) => e.localId === etapeId ? {
      ...e, images: e.images.map((img) => img.localId === imageId ? { ...img, description } : img),
    } : e));
  };
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      for (const [i, etape] of etapes.entries()) {
        const er = await api.post("/etapes/", { version: version.id, numero: i + 1, titre: etape.titre.trim(), description: etape.description.trim() || null, duree_minutes: etape.duree_minutes, ordre: i + 1 });
        ...
        for (const [j, image] of etape.images.entries()) await api.post("/images-etapes/", { etape: er.data.id, image_url: image.image_url, description: image.description.trim() || null, ordre: j + 1 });
      }
  ...
  return (
    <form className="gamme-create-page" onSubmit={save}>
      ...
      <section className="wizard-card">
        ...
        {currentStep === 4 && (
          <>
            <div className="section-heading"><h2>Étapes de maintenance</h2><p>Définissez les opérations, actions obligatoires, durées et images.</p></div>
            ...
            {etapes.length === 0 ? <div className="empty-state-box">Aucune étape. Cliquez sur « Ajouter une étape ».</div> : <div className="maintenance-steps-list">{etapes.map((etape) => <article key={etape.localId} className="maintenance-step-card">
              <div className="maintenance-step-header"><div className="step-number-badge">{etape.numero}</div><div className="maintenance-step-heading"><strong>Étape {etape.numero}</strong><span>{etape.duree_minutes} min</span></div><button type="button" className="danger-icon-button" onClick={() => removeEtape(etape.localId)}><Trash2 size={16}/></button></div>
              <div className="step-main-fields"><label><span>Titre *</span><input value={etape.titre} onChange={(e) => updateEtape(etape.localId,"titre",e.target.value)}/></label><label><span>Durée (min)</span><input type="number" min={0} value={etape.duree_minutes} onChange={(e) => updateEtape(etape.localId,"duree_minutes",e.target.value)}/></label><label className="step-description-field"><span>Description</span><textarea rows={3} value={etape.description} onChange={(e) => updateEtape(etape.localId,"description",e.target.value)}/></label></div>
              <div className="step-subsection"><div className="step-subsection-title"><div><strong>Actions obligatoires</strong><span>Au moins une action renseignée.</span></div><button type="button" className="secondary-button" onClick={() => addAction(etape.localId)}><Plus size={15}/> Ajouter une action</button></div><div className="actions-editor-list">{etape.actions.map((a,j) => <div key={a.localId} className="action-editor-row"><div className="action-order">{j+1}</div><textarea rows={2} value={a.contenu} onChange={(e) => updateAction(etape.localId,a.localId,e.target.value)} placeholder="Action à réaliser *"/><button type="button" className="danger-icon-button" onClick={() => removeAction(etape.localId,a.localId)}><Trash2 size={15}/></button></div>)}</div></div>
              <div className="step-subsection"><div className="step-subsection-title"><div><strong>Images</strong><span>Photos avec description.</span></div><label className="secondary-button step-file-button"><ImagePlus size={15}/> Ajouter une image<input type="file" accept="image/*" onChange={(e) => { addEtapeImage(etape.localId,e.target.files?.[0]); e.currentTarget.value=""; }}/></label></div>{etape.images.length === 0 ? <div className="step-images-empty">Aucune image.</div> : <div className="step-images-grid">{etape.images.map((img) => <div key={img.localId} className="step-image-card"><div className="step-image-preview"><img src={img.image_url} alt="Étape"/><button type="button" className="step-image-remove" onClick={() => removeEtapeImage(etape.localId,img.localId)}><X size={15}/></button></div><textarea rows={2} value={img.description} onChange={(e) => updateEtapeImageDescription(etape.localId,img.localId,e.target.value)} placeholder="Description de l'image"/></div>)}</div>}</div>
            </article>)}</div>}
          </>
        )}
